...
int zc_profile_inner(int flag, const char *file, const long line, const char *fmt, ...)
{
	...
	switch (flag)
	{
		case ZC_DEBUG:
			...
			fprintf(fp, "%s DEBUG  (pid:%d file:%s line:%ld) ",time_str, getpid(), file, line);
			break;
		case ZC_WARN:
			...
			if(!fp)return -1;
			zc_time(time_str, sizeof(time_str));
			fprintf(fp, "%s WARN  (pid:%d file:%s line:%ld) ",time_str, getpid(), file, line);
			break;
		case ZC_ERROR:
			if (error_log == NULL)return 0;
			fp = fopen(error_log,"a");
			if(!fp)return -1;
			zc_time(time_str, sizeof(time_str));
			fprintf(fp, "%s ERROR  (pid:%d file:%s line:%ld) ",time_str, getpid(), file, line);
			break;
	}
